power_simulation_mpi <- 
  function (model, data, simvar, fixed_effects, critical_value, 
            ...
            R2 = F, R2var, R2level, nCores=cores_num(hosts="hosts"), 
            chunkSize=floor(n_sim/(2*hosts_num(hosts="hosts"))))
  {
    ...
    print(cl)
    if (safeguard == T) {
      model_for_simulation <- mixedpower:::prepare_safeguard_model(model, 
    ...
    for (n in steps) {
      ...
      store_simulations <- suppressWarnings(foreach::foreach(iterators::icount(n_sim), 
                                              ...
                                              {
                                                if (rnorm == T) {
                                                  model_for_simulation <- mixedpower:::prepare_rnorm_model(model, 
                                                ...
                                                final_dataset <- mixedpower:::reset_contrasts(simulated_data, 
                                                ...
                                                if (R2 == F) {
                                                  to.store_simulations <- mixedpower:::check_significance(model_final, 
                                                                                                          critical_value)
                                                }
                                                else {
                                                  to.store_simulations <- mixedpower:::check_significance(model_R2_final, 
                                                                                                          critical_value)
                                                }
                                                return(to.store_simulations)
                                              })
      print(paste("Simulations for step ", n, " are based on ", 
                  length(store_simulations)/n_row, " successful single runs"))
      print(paste0("store_simulations: ",store_simulations))
